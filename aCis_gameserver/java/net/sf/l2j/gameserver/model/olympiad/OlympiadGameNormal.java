	...
	protected void validateWinner(OlympiadStadiumZone stadium)
	{
		...
		if (_playerOne.isDefecting() || _playerTwo.isDefecting())
		{
			if (_playerOne.isDefecting())
				removePointsFromParticipant(_playerOne, Math.min(playerOnePoints / 3, Config.ALT_OLY_MAX_POINTS));
			
			if (_playerTwo.isDefecting())
				removePointsFromParticipant(_playerTwo, Math.min(playerTwoPoints / 3, Config.ALT_OLY_MAX_POINTS));
		...
		final int pointDiff = MathUtil.limit(Math.min(playerOnePoints, playerTwoPoints) / getDivider(), 1, Config.ALT_OLY_MAX_POINTS);
