...
public class PcInventory extends Inventory
{
	...
	public boolean validateCapacityByItemId(int itemId, int itemCount)
	{
		...
		return validateCapacity(item.isStackable() ? 1 : itemCount);
	}
	
	public boolean validateCapacityByItemIds(List<IntIntHolder> items)
	{
		int slots = 0;
		for (IntIntHolder item : items)
		{
			final ItemInstance invItem = getItemByItemId(item.getId());
			if (invItem != null && invItem.isStackable())
				continue;
			
			final Item i = ItemData.getInstance().getTemplate(item.getId());
			slots += i.isStackable() ? 1 : item.getValue();
		}
	...
	{
		return (_items.size() + slots <= _owner.getStatus().getInventoryLimit());
	}
