...
public class PlayerCast extends PlayableCast<Player>
{
	...
	public boolean canDoCast(Creature target, L2Skill skill, boolean isCtrlPressed, int itemObjectId)
	{
		...
		switch (skillType)
		{
			...
			case TAKECASTLE:
				if (!_actor.checkIfOkToCastSealOfRule(CastleManager.getInstance().getCastle(_actor), false, skill, target))
					return false;
				
				break;
			
			case SIEGEFLAG:
				if (!L2SkillSiegeFlag.checkIfOkToPlaceFlag(_actor, false))
					return false;
				
				break;
			
			case STRSIEGEASSAULT:
				if (!_actor.checkIfOkToUseStriderSiegeAssault(skill))
					return false;
				
				break;
			
			case SUMMON_FRIEND:
				if (!(_actor.checkSummonerStatus() && _actor.checkSummonTargetStatus(target)))
					return false;
